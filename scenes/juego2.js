export default class juego2 extends Phaser.Scene {
  ...
  preload() {
    ...
    this.load.image("noche", "./public/assets/fondodenoche.png");
  ...
  create() {
    ...
    console.log(this.registry.get('puntaje'));
    ...
    this.add.image(width / 2, height / 2, "noche");
    ...
    this.textoPuntaje = this.add.text(scoreImg.x + scoreImg.displayWidth + 10, scoreImg.y, this.puntaje, { fontSize: "48px", fill: "#fff", fontFamily: "times new roman" }).setOrigin(0, 0.5);

    this.tiempoGlobal = this.registry.get('tiempoGlobal') || 20;
    ...
    this.timerEvent = this.time.addEvent({
      ...
      callback: () => {
        ...
        if (this.tiempoGlobal >= 60 && !this.finalizado) {
          this.finalizado = true;
          this.physics.world.pause();
          this.stop = true;

          const fondo = this.add.rectangle(0, 0, this.sys.game.config.width, this.sys.game.config.height, 0x000000, 0.7);
          fondo.setOrigin(0, 0);
          const cartelTiempo = this.add.image(this.cameras.main.centerX, this.cameras.main.centerY, "tiempoterminado").setOrigin(0.5);
          const scaleX = this.sys.game.config.width / cartelTiempo.width;
          const scaleY = this.sys.game.config.height / cartelTiempo.height;
          cartelTiempo.setScale(Math.max(scaleX, scaleY)).setDepth(10);
          this.add.text(this.cameras.main.centerX - 70, this.cameras.main.centerY + 90, this.registry.get('puntaje'), { fontSize: "120px", fill: "#000", fontFamily: "pixelify sans", fontStyle: "bold" }).setOrigin(0.5).setDepth(11);
          const botonSiguiente = this.add.image(this.cameras.main.centerX, this.cameras.main.centerY + 280, "botonsiguiente");
          botonSiguiente.setInteractive({ useHandCursor: true });
          botonSiguiente.setScale(2.8).setDepth(12);
          botonSiguiente.on("pointerdown", () => {
            this.registry.set('tiempoGlobal', this.tiempoGlobal);
    ...
    if (this.cargasLaser === 0 && this.fuerza > 0) {
      ...
      if (!this.finalizado) {
        ...
        personaje.once("animationcomplete-golpeado", () => {
          ...
          this.add.text(this.cameras.main.centerX + 170, this.cameras.main.centerY + 90, this.puntaje, {
            fontSize: "120px",
            fill: "#000",
